...
class GetCT(Dataset):
    ...
    def __getitem__(self,index):
        ...
        y = skimage.io.imread(self.data_names[index])
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetCT(root= "/code/dog_train_128")
  val_dataset = GetCT(root= "/code/dog_train_128")
  
  
  train_ds = DataLoader(dataset, batch_size=config.training.batch_size, shuffle=True,
                                num_workers=0)
                                
  eval_ds = DataLoader(val_dataset, batch_size=config.eval.batch_size, shuffle=True,
